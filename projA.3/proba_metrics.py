...
def calc_mean_binary_cross_entropy_from_probas(ytrue_N, yproba1_N):
    ...
    bce_prob = np.mean(-ytrue_N * np.log2(yproba1_N) - (1-ytrue_N) * np.log2(1-yproba1_N))
...
def calc_mean_binary_cross_entropy_from_scores(ytrue_N, scores_N):
    ...
    # Correctly flip the scores
    flipped_scores_N = yflippedsign_N * scores_N

    # Calculate BCE using the logsumexp trick for numerical stability
    # This computes log(1 + exp(flipped_score)) directly for each score
    # And then we take the average
    bce_score = np.mean(np.logaddexp(0, flipped_scores_N) / np.log(2))
